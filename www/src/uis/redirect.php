...
if (count($_POST)){
    // $uis_cont->logger->log('Data in POST: '.json_encode($_POST));
    $hook_data = $_POST;
} elseif (strlen($input)){
    $input_data = json_decode($input,true) ?? null;
    if (is_array($input_data) && count($input_data)){
        // $uis_cont->logger->log('Data in input: '.json_encode($input_data));
        $hook_data = $input_data;
    }
} elseif (count($_GET)){
    // $uis_cont->logger->log('Data in GET: '.json_encode($_GET));
    $hook_data = $_GET;
}
if ($hook_data) {
    $result = $uis_cont->processRedirect($hook_data);
